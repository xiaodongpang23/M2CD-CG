...
set M20xyz [[atomselect top "segname M20"] get {x y z}]
set M20TMxyz [[atomselect top "segname M20 and resid 4 to 25 45 to 66 86 to 107 127 to 148"] get {x y z}]
...
set outfileM20 [open "DAT/M20allxyzf${frame}.dat" w]
set outfileM20TM [open "DAT/M20TMallxyzf${frame}.dat" w]
puts $outfileup "X\tY\tZ" 
puts $outfilelow "X\tY\tZ" 
puts $outfileM2 "X\tY\tZ" 
puts $outfileM20 "X\tY\tZ" 
puts $outfileM20TM "X\tY\tZ" 

foreach XYZ $M20xyz {
	set xi [lindex $XYZ 0] 	
	set yi [lindex $XYZ 1] 	
	set zi [lindex $XYZ 2] 	
	puts $outfileM20 "$xi\t$yi\t$zi" 
}

foreach XYZ $M20TMxyz {
	set xi [lindex $XYZ 0] 	
	set yi [lindex $XYZ 1] 	
	set zi [lindex $XYZ 2] 	
	puts $outfileM20TM "$xi\t$yi\t$zi" 
}
